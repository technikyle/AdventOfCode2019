...
//  day3
//
//  Created by Kyle Rokita on 12/3/19.
...
class ViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        func program(wire1: String, wire2: String) -> Int {
            ...
            func enumeratePositions(wire: [(String, Int)], work: (Int, Int) -> Bool) {
                ...
                for direction in wire {
                    ...
                    while distance != 0 {
                        ...
                        position = (position.x + xDirection, position.y + yDirection)

                        let shouldContinue = work(position.x, position.y)
            ...
            func testWires(a: [(String, Int)], b: [(String, Int)]) -> [Int] {

                //        var grid = Array<[Int]>(repeating: Array<Int>(repeating: 0, count: max * 2), count: max * 2)

                var map = [String: Int]()

                print("wow")

                func gridHash(x:Int, y:Int) -> String {
                    return String(x) + ":" + String(y)
                }

                var distance: Int = 0
                // setup grid for wire 1
                enumeratePositions(wire: a) { x, y -> Bool in
                    distance += 1
                    map[gridHash(x:x, y:y)] = distance
                    return true
                }

                // search for intersection with wire2

                var results = [Int]()
                var distance2: Int = 0
                enumeratePositions(wire: b) { x, y -> Bool in
                    distance2 += 1
                    var value = map[gridHash(x:x, y:y)]
                    if let value = value {
                        //                print((x:x,y:y))
                        results.append(value + distance2)
                    }
                    return true
                }
            ...
            let results = testWires(a: wire1Directions, b: wire2Directions)

            //    let distance: ((x: Int, y: Int)) -> Int = { element -> Int in
            //        return abs(element.x) + abs(element.y)
            //    }

            //    var overall = results.reduce(into: (x:max, y: max)) { result, element in
            //
            //        result =  distance(element) < distance(result) ? element: result
            //    }

            return results.min()!
        }

        print(program(wire1: wire1, wire2: wire2))
    }
